...
class AesEncryptECB:
    ...
    mac下 pip install pycryptodome -i https://pypi.doubanio.com/simple
    """
...
if __name__ == '__main__':
    ...
    # '5cf57d8e9d9fb162'
    aes = AesEncryptCBC(key='1234567890123456',iv='5cf57d8e9d9fb162')
    data = 'python自动化'
    res = aes.encrypt(data)
    print(res)
    print(aes.decrypt(res))
    ...
    rsa = RsaEncrypt(public_file='publick_key.keystore',private_file='private_key.keystore')
    # 为了调试rsa，我们先生成公钥和私钥，在公司里不用自己生成得找开发问
    # rsa.generate_key()
    data = rsa.encrypt_data('python全栈自动化测试')
    print(data)
